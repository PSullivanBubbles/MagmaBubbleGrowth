...
def growBubble(volumeScale):
    ...
    #Spatial parameters
    Nb = 1e11/volumeScale; #Bubble number density (number per m^3)
    ...
    switch PTtModel
        ...
        case 'P: Isobaric, T: Polythermal-Dwell'
            ...
            T_f = 1200 + 273.15; %Final temperature in K
            dTdt = 0.1; %Rate of temperature change in K/s
            P_0 = 20 * 1e6; %Initial pressure in Pa (MPa to Pa)
            P_f = [];%Final pressure in Pa (MPa to Pa)
            dPdt = []; %Rate of pressure change in Pa S (Mpa to Pa)
            H2Ot_0 = 1.6; %initial water concentration (wt. %)
            t_nuc = 264.6; %Nucleation time
            t_f = 3000; %Experiment duration (seconds)
